<template>
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" title="批量操作任务详情"> <!-- CURD详情页--> </n-card>
    </div>
    <n-card :bordered="false" class="proCard mt-4" size="small" :segmented="{ content: true }">
      <n-descriptions label-placement="left" class="py-2" column="4">
        ...
        <n-descriptions-item>
          <template #label>操作动作</template>
          {{ formValue.action }}
        </n-descriptions-item>

        <n-descriptions-item>
          <template #label>账号 id</template>
          {{ formValue.accounts }}
        </n-descriptions-item>

        <n-descriptions-item>
          <template #label>执行任务参数</template>
          {{ formValue.parameters }}
        </n-descriptions-item>

        <n-descriptions-item label="任务状态,1运行,2停止,3完成,4失败">
          <n-tag
...
<script lang="ts" setup>
  ...
  import { View } from '@/api/tg/tgBatchExecutionTask';
