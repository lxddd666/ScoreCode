  ...
  <div>
    ...
    <n-card :bordered="false" class="proCard mt-4" size="small" :segmented="{ content: true }">
      <n-descriptions label-placement="left" class="py-2" column="4">
        ...
        <n-descriptions-item>
          <template #label>已分配账号数量</template>
          {{ formValue.assignedCount }}
        </n-descriptions-item>

        <n-descriptions-item>
          <template #label>长期未登录数量</template>
          {{ formValue.longTermCount }}
        </n-descriptions-item>

        <n-descriptions-item>
          <template #label>地区</template>
          <span v-html="formValue.region"></span></n-descriptions-item>
...
<script lang="ts" setup>
  ...
  import { View } from '@/api/tg/tgProxy';
