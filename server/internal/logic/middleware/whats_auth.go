...
// WhatsAuth 后台鉴权中间件
func (s *sMiddleware) WhatsAuth(r *ghttp.Request) {
	var (
		ctx  = r.Context()
		path = gstr.Replace(r.URL.Path, simple.RouterPrefix(ctx, consts.AppWhats), "", 1)
	)

	// 不需要验证登录的路由地址
	if s.IsExceptLogin(ctx, consts.AppWhats, path) {
		r.Middleware.Next()
		return
	}
	...
	// 不需要验证权限的路由地址
	if s.IsExceptAuth(ctx, consts.AppWhats, path) {
		r.Middleware.Next()
		return
	}
