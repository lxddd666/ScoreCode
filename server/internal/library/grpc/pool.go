...
func New(address string, option Options) (Pool, error) {
	...
	for i := 0; i < p.opt.MaxIdle; i++ {
		c := p.opt.Dial(address)

		p.conns[i] = p.wrapConn(c, false)
	}
	g.Log().Printf(ctx, "new pool success: %v\n", p.Status())
...
func (p *pool) Get() (Conn, error) {
	...
	if current < int32(p.opt.MaxActive) && nextRef > current*int32(p.opt.MaxConcurrentStreams) {
		...
		current += i
		g.Log().Printf(ctx, "grow pool: %d ---> %d, increment: %d, maxActive: %d\n",
...
func (p *pool) Close() error {
	...
	p.deleteFrom(0)
	g.Log().Printf(ctx, "close pool success: %v\n", p.Status())
	return nil
}
