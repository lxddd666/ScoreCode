...
type RegisterInp struct {
	...
	Mobile     string `json:"mobile" v:"required-without:Email#手机号邮箱不能同时为空" dc:"手机号,邮箱为空时必填"`
	Email      string `json:"email" v:"required-without:Mobile|email#手机号邮箱不能同时为空|邮箱格式不正确"  dc:"邮箱,手机号为空时必填"`
