...
func (in *TgBatchExecutionTaskEditInp) Filter(ctx context.Context) (err error) {
	...
	// 验证执行任务参数
	if err := g.Validator().Rules("required").Data(in.Parameters).Messages(g.I18n().T(ctx, "{#ExecutionNotEmpty}")).Run(ctx); err != nil {
		return err.Current()
	}
...
type TgBatchExecutionTaskImportSessionLogInp struct {
	Id int64 `json:"id" v:"required#IdNotEmpty" dc:"task ID"`
}
