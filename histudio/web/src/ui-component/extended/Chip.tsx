// material-ui
import { useTheme } from '@mui/material/styles';
import MuiChip, { ChipProps } from '@mui/material/Chip';

// ==============================|| CHIP ||============================== //

interface chipProps extends ChipProps {
    chipcolor?: string;
    sx?: {};
    disabled?: boolean;
    label?: string;
    avatar?: React.ReactElement | undefined;
    maxWidth?: string;
    onDelete?: () => void;
    onClick?: () => void;
}

const Chip = ({ chipcolor, disabled, sx = {}, variant, maxWidth, ...others }: chipProps) => {
    const theme = useTheme();

    let defaultSX = {
        borderRadius: 1,
        maxWidth: maxWidth,
        color: theme.palette.mode === 'dark' ? theme.palette.primary.light : theme.palette.primary.main,
        bgcolor: theme.palette.mode === 'dark' ? theme.palette.primary.main : theme.palette.primary.light,
        ':hover': {
            color: theme.palette.primary.light,
            bgcolor: theme.palette.mode === 'dark' ? theme.palette.primary.dark + 90 : theme.palette.primary.dark
        }
    };

    let outlineSX = {
        color: theme.palette.primary.main,
        bgcolor: 'transparent',
        border: '1px solid',
        borderRadius: 1,
        maxWidth: maxWidth,
        borderColor: theme.palette.primary.main,
        ':hover': {
            color: theme.palette.mode === 'dark' ? theme.palette.primary.light : theme.palette.primary.light,
            bgcolor: theme.palette.mode === 'dark' ? theme.palette.primary.main : theme.palette.primary.dark
        }
    };

    switch (chipcolor) {
        case 'primary':
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.primary.main,
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.primary.main,
                    ':hover': {
                        color: theme.palette.mode === 'dark' ? theme.palette.primary.light : theme.palette.primary.main,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.primary.dark : theme.palette.primary.light
                    },
                })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.mode === 'dark' ? theme.palette.primary.light : theme.palette.primary.main,
                    bgcolor: theme.palette.mode === 'dark' ? theme.palette.primary.dark : theme.palette.primary.light,
                    ':hover': {
                        color: theme.palette.primary.light,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.primary.dark + 90 : theme.palette.primary.main
                    }
                });
            break;
        case 'secondary':
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.secondary.main,
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.secondary.main,
                    ':hover': {
                        color: theme.palette.mode === 'dark' ? theme.palette.secondary.light : theme.palette.secondary.main,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.secondary.dark : theme.palette.secondary.light
                    }
                })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.mode === 'dark' ? theme.palette.secondary.light : theme.palette.secondary.main,
                    bgcolor: theme.palette.mode === 'dark' ? theme.palette.secondary.dark : theme.palette.secondary.light,
                    ':hover': {
                        color: theme.palette.secondary.light,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.secondary.dark + 90 : theme.palette.secondary.main
                    }
                });
            break;
        case 'success':
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.success.dark,
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.success.dark,
                    ':hover': {
                        color: theme.palette.mode === 'dark' ? theme.palette.success.light : theme.palette.success.dark,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.success.dark : theme.palette.success.light + 60
                    }
                })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.mode === 'dark' ? theme.palette.success.light : theme.palette.success.dark,
                    bgcolor: theme.palette.mode === 'dark' ? theme.palette.success.dark : theme.palette.success.light + 60,
                    ':hover': {
                        color: theme.palette.success.light,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.success.dark + 90 : theme.palette.success.dark
                    }
                });
            break;
        case 'error':
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.error.main,
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.error.main,
                    ':hover': {
                        color: theme.palette.mode === 'dark' ? theme.palette.error.light : theme.palette.error.dark,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.error.dark : theme.palette.error.light
                    }
                })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.mode === 'dark' ? theme.palette.error.light : theme.palette.error.dark,
                    bgcolor: theme.palette.mode === 'dark' ? theme.palette.error.dark : theme.palette.error.light + 60,
                    ':hover': {
                        color: theme.palette.error.light,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.error.dark + 90 : theme.palette.error.dark
                    }
                });
            break;
        case 'orange':
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.orange?.dark!,
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.orange?.main!,
                    ':hover': {
                        color: theme.palette.orange?.dark!,
                        bgcolor: theme.palette.orange?.light!
                    }
                })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.orange?.dark!,
                    bgcolor: theme.palette.orange?.light!,
                    ':hover': {
                        color: theme.palette.orange?.light!,
                        bgcolor: theme.palette.orange?.dark!
                    }
                });
            break;
        case 'warning':
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.warning.dark,
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.warning.dark,
                    ':hover': {
                        color: theme.palette.warning.dark,
                        bgcolor: theme.palette.warning.light
                    }
                })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.warning.dark,
                    bgcolor: theme.palette.warning.light,
                    ':hover': {
                        color: theme.palette.warning.light,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.warning.dark + 90 : theme.palette.warning.dark
                    }
                });
            break;
        case 'info':
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.info.dark,
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.info.dark,
                    ':hover': {
                        color: theme.palette.mode === 'dark' ? theme.palette.info.light : theme.palette.info.dark,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.info.dark : theme.palette.info.light + 60
                    }
                })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.mode === 'dark' ? theme.palette.info.light : theme.palette.info.dark,
                    bgcolor: theme.palette.mode === 'dark' ? theme.palette.info.dark : theme.palette.info.light + 60,
                    ':hover': {
                        color: theme.palette.info.light,
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.info.dark + 90 : theme.palette.info.dark
                    }
                });
            break;
        default:
            variant === 'outlined'
                ? (outlineSX = {
                    ...outlineSX,
                    color: theme.palette.grey[400],
                    bgcolor: 'transparent',
                    border: '1px solid',
                    borderRadius: 1,
                    borderColor: theme.palette.grey[400],
                    ':hover': {
                        color: theme.palette.mode === 'dark' ? theme.palette.grey[100] : theme.palette.grey[400],
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.grey[700] : theme.palette.grey[100]
                    }
                  })
                : (defaultSX = {
                    ...defaultSX,
                    borderRadius: 1,
                    color: theme.palette.mode === 'dark' ? theme.palette.grey[100] : theme.palette.grey[400],
                    bgcolor: theme.palette.mode === 'dark' ? theme.palette.grey[700] : theme.palette.grey[100],
                    ':hover': {
                        color: theme.palette.grey[100],
                        bgcolor: theme.palette.mode === 'dark' ? theme.palette.grey[700] + 90 : theme.palette.grey[400]
                    }
                });
            break;
    }

    if (disabled) {
        variant === 'outlined'
            ? (outlineSX = {
                ...outlineSX,
                color: theme.palette.grey[500],
                bgcolor: 'transparent',
                border: '1px solid',
                borderRadius: 1,
                borderColor: theme.palette.grey[500],
                ':hover': {
                    color: theme.palette.grey[500],
                    bgcolor: 'transparent'
                }
            })
            : (defaultSX = {
                ...defaultSX,
                borderRadius: 1,
                color: theme.palette.grey[500],
                bgcolor: theme.palette.grey[50],
                ':hover': {
                    color: theme.palette.grey[500],
                    bgcolor: theme.palette.grey[50]
                }
            });
    }

    let SX = defaultSX;
    if (variant === 'outlined') {
        SX = outlineSX;
    }
    SX = { ...SX, ...sx };
    return <MuiChip {...others} sx={SX} />;
};

export default Chip;
