    ...
    useEffect(() => {
        ...
        if (menu.children) {
            menu.children.forEach((item: NavItemType) => {
                ...
                let pathnames = window.location.pathname.toLowerCase().split('/');
                if (pathnames.length > 1 && item.url) {
                    let itemPaths = item.url!.toLowerCase().split('/');
                    if (pathnames[1].toLowerCase() === itemPaths[1]) {
                        setSelected(menu.id);
                        setOpen(true);
