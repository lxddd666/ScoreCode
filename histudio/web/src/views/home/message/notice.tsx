...
const Notice = () => {
    ...
    const initialQueryParamString: string[] = [`page=1`, `pageSize=${rowsPerPage}`, `type=1`];
    const [queryParamString, setQueryParamString] = React.useState<String[]>(initialQueryParamString);

    React.useEffect(() => {
        setQueryParamString([`page=${page + 1}`, `pageSize=${rowsPerPage}`, `type=1`]);
    }, [page, rowsPerPage]);
    ...
    const handleChangePage = (event: React.MouseEvent<HTMLButtonElement, MouseEvent> | null, newPage: number) => {
        setPage(newPage);
        fetchData([`page=${newPage + 1}`, `pageSize=${rowsPerPage}`, `type=1`]);
    };
    ...
    const handleChangeRowsPerPage = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement> | undefined) => {
        ...
        fetchData([`page=${page + 1}`, `pageSize=${val}`, `type=1`]);
    };
                                            ...
                                            <Grid container spacing={gridSpacing}>
                                                <Grid item xs={2}>
                                                    <StyledBadge badgeContent={row.isRead ? undefined : ''} color="error">
                                                        <Avatar sizes="medium">
                                                            <CampaignTwoToneIcon sx={{ fontSize: '1.3rem' }} />
                                                        </Avatar>
                                                    </StyledBadge>
                                                </Grid>
                                                <Grid item xs={8}>
