...
const Announcement = () => {
    ...
    const initialQueryParamString: string[] = [`page=1`, `pageSize=${rowsPerPage}`, `type=2`];
    const [queryParamString, setQueryParamString] = React.useState<String[]>(initialQueryParamString);

    React.useEffect(() => {
        setQueryParamString([`page=${page + 1}`, `pageSize=${rowsPerPage}`, `type=2`]);
    }, [page, rowsPerPage]);
    ...
    const handleChangePage = (event: React.MouseEvent<HTMLButtonElement, MouseEvent> | null, newPage: number) => {
        setPage(newPage);
        fetchData([`page=${newPage + 1}`, `pageSize=${rowsPerPage}`, `type=2`]);
    };
    ...
    const handleChangeRowsPerPage = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement> | undefined) => {
        ...
        fetchData([`page=${page + 1}`, `pageSize=${val}`, `type=2`]);
    };
    ...
    function TablePaginationActions(props: TablePaginationActionsProps) {
        ...
        return (
            <Grid container spacing={2} alignItems="center" justifyContent="flex-end" marginLeft={1} className="customTablePagination">
                                            ...
                                            <Grid container spacing={gridSpacing}>
                                                <Grid item xs={2}>
                                                    <StyledBadge badgeContent={row.isRead ? undefined : ''} color="error">
                                                        <Avatar sizes="medium">
                                                            <NotificationImportantTwoToneIcon sx={{ fontSize: '1.3rem' }} />
                                                        </Avatar>
                                                    </StyledBadge>
                                                </Grid>
                                                <Grid item xs={8}>
