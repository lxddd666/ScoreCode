...
import { PostListData } from 'types/subcron';
import { ResponseList } from 'types/response';
import { useDispatch, useSelector } from 'store';
// cron list api
import { getPostList } from 'store/slices/subcron';
...
const CronGroupWrapper = styled(MainCard)(({
    overflow: 'auto'
}));
...
const headCells: HeadCell[] = [
    ...
    },
    {
        id: 'assignTask',
        numeric: false,
        label: 'setting.crongroup.superior-id',
        align: 'center'
    },
    {
        id: 'taskName',
        numeric: false,
        label: 'setting.crongroup.group-name',
        align: 'center'
    },
    {
        id: 'participant',
        numeric: false,
        label: 'setting.crongroup.is-default',
        align: 'center'
    },
    {
        id: 'planning',
...
export const CronGroup = () => {
    ...
    const { postList } = useSelector((state) => state.subcron);
    ...
    const handleAddModal = (id?: number) => {
        if (id) {
            axiosServices
                .get(`${envRef?.API_URL}admin/cronGroup/view?id=${id}`, { headers: {} })
                .then(function (response) {
                    if (response?.data?.code === 0) {
    ...
    const handleActivate = (id: number, status: number) => {
        axiosServices
            .post(
                `${envRef?.API_URL}admin/cronGroup/status`,
                {
                    id: id,
                    status: status === 1 ? 2 : 1
                },
    ...
    const handleDelete = (ids: number[]) => {
        axiosServices
            .post(`${envRef?.API_URL}admin/cronGroup/delete`, { id: ids }, { headers: {} })
            .then(function (response) {
                if (response?.data?.code === 0) {
                    dispatch(
                        openSnackbar({
                            ...
                            stableSort(rows ? rows : [], getComparator(order, orderBy)).map((row, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="center">否</TableCell>{/* hard code */}
                                        ...
                                        <TableCell align="center">
                                            ...
                                            {row.status === 2 && (
                                                <Chip
                                                    label={intl.formatMessage({ id: 'setting.crongroup.disable' })}
                                                    size="small"
                                                    chipcolor="orange"
                                                />
                                            )}
...
// page num footer
    ...
    return (
        <CronGroupWrapper title={<FormattedMessage id="setting.crongroup.management-grouping" />} content={false}>
            <CardContent>
                <Grid container alignItems="center" spacing={gridSpacing} justifyContent="flex-start">
                    <Grid item xs={12} sm={4} md={3} sx={{ display: 'flex', justifyContent: 'flex-end', marginLeft: 'auto' }}>
                        <AddButton onClick={() => handleAddModal()} tooltipTitle={intl.formatMessage({ id: 'setting.crongroup.add-group' })} />
                    </Grid>
                </Grid>
            </CardContent>
                    ...
                    <>
                        <Formik
                            ...
                            validationSchema={Yup.object().shape({
                                name: Yup.string()
                                    .max(255)
                                    .required(intl.formatMessage({ id: 'setting.crongroup.enter-group-name' })),
                            })}
                            onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
                                try {
                                    await axiosServices
                                        .post(
                                            `${envRef?.API_URL}admin/cronGroup/edit`,
                                            {
                                                id: values.id || 0,
                                                pid: values.pid || 0,
                                                name: values.name,
                                ...
                                <form noValidate onSubmit={handleSubmit}>
                                    <Grid container alignItems="center" spacing={gridSpacing}>
                                        <Grid item className="expandableSelectContainer" xs={12} sm={6}>
                                            ...
                                            >
                                                <ExpandableSelect
                                                    label={intl.formatMessage({ id: 'setting.crongroup.superior-group' })}
                                                    id="pid"
                                            ...
                                            <FormattedMessage id="setting.crongroup.selection-info" />
                                        </Grid>
                                        <Grid item xs={12} sm={6}>
                                            ...
                                            >
                                                <InputLabel htmlFor="outlined-adornment-code-add">
                                                    {intl.formatMessage({ id: 'setting.crongroup.group-name' })}
                                                </InputLabel>
                                                <OutlinedInput
