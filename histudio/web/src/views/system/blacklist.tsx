...
// project imports
import { BlackListData, BlackList1 } from 'types/blacklist';
import { useDispatch, useSelector } from 'store';
import { getBlackList } from 'store/slices/blacklist';
...
function stableSort(array: BlackListData[], comparator: (a: BlackListData, b: BlackListData) => number) {
    const stabilizedThis = array.map((el: BlackListData, index: number) => [el, index]);
    stabilizedThis.sort((a, b) => {
        const order = comparator(a[0] as BlackListData, b[0] as BlackListData);
...
const headCells: HeadCell[] = [
    ...
    },
    {
        id: 'status',
        numeric: false,
        label: 'setting.blacklist.status',
        align: 'center'
    },
    {
        id: 'remark',
        numeric: false,
        label: 'setting.blacklist.remark',
        align: 'center'
    },
    {
        id: 'createdAt',
        numeric: false,
        label: 'setting.blacklist.created-date',
        align: 'center'
    },
    {
        id: 'updatedAt',
            ...
            <TableRow>
                ...
                {numSelected <= 0 && (
                    <TableCell sortDirection={false} align="center" className="sticky" sx={{ pr: 3, right: '0' }}>
                        {intl.formatMessage({ id: 'setting.blacklist.control' })}
                    </TableCell>
                )}
            </TableRow>
        </TableHead>
    ...
    >
        {numSelected > 0 ? (
            ...
            <Typography
                ...
                id="tableTitle"
            >
                <FormattedMessage id="setting.blacklist.blacklist" />
            </Typography>
        )}
        <Box sx={{ flexGrow: 2 }} />
        {numSelected > 0 && (
...
const Blacklist = () => {
    ...
    const initSearchFields: SearchFields = { ip: '', status: 0, remark: '', created_at: [] };
    ...
    const [rows, setRows] = React.useState<BlackListData[]>();
    const [res, setRes] = React.useState<BlackList1>();
    ...
    const fetchData = async (queries: String[]) => {
        try {
            setLoading(true);
            setRows(undefined);
            await dispatch(getBlackList(queries.filter((query) => !query.endsWith('=') && query !== 'status=0').join('&')));
        } catch (error) {
            dispatch(
                openSnackbar({
                    open: true,
    ...
    const handleAddModal = (id?: number) => {
        if (id) {
            axiosServices
                .get(`${envRef?.API_URL}/admin/blacklist/view?id=${id}`, { headers: {} })
    ...
    const statusOptions = [
        ...
        },
        {
            id: 1,
            value: 1,
            name: intl.formatMessage({ id: 'setting.blacklist.blocked' })
        },
        {
            id: 2,
            value: 2,
            name: intl.formatMessage({ id: 'setting.blacklist.unblocked' })
        }
    ];
    ...
    const handleActivate = (id: number, status: number) => {
        axiosServices
            .post(
                `${envRef?.API_URL}/admin/blacklist/status`,
                {
                    id: id,
                    /*
    ...
    const [selectedToDelete, setSelectedToDelete] = React.useState<number[]>();
    ...
    const handleDelete = (ids: number[]) => {
        axiosServices
            .post(`${envRef?.API_URL}/admin/blacklist/delete`, { id: ids }, { headers: {} })
            .then(function (response) {
                if (response?.data?.code === 0) {
                    dispatch(
                        openSnackbar({
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="center">
                                            {row.status === 1 && (
                                                <Chip
                                                    label={intl.formatMessage({ id: 'setting.blacklist.blocked' })}
                                                    size="small"
                                                    chipcolor="orange"
                                                />
                                            )}
                                            {row.status === 2 && (
                                                <Chip
                                                    label={intl.formatMessage({ id: 'setting.blacklist.unblocked' })}
                                                    size="small"
                                                    chipcolor="success"
                                                />
                                            )}
                                        ...
                                        {handleLastActiveDate(row.updatedAt)}
    ...
    return (
        <MainCard title={<FormattedMessage id="setting.blacklist.blacklist" />} content={false}>
            <CardContent>
                <Grid container alignItems="center" spacing={2} justifyContent="flex-start">
                    <Grid item xs={12} sm={3}>
                        <TextField
                            ...
                            placeholder={intl.formatMessage({ id: 'setting.blacklist.search-ip' })}
                            value={search.ip}
                            size="small"
                            label={<FormattedMessage id="setting.blacklist.ip" />}
                        />
                    </Grid>
                    <Grid item xs={12} sm={3}>
                        <TextField
                            ...
                            onChange={(event) => handleSearch(event, 'remark')}
                            placeholder={intl.formatMessage({ id: 'setting.blacklist.search-remark' })}
                            value={search.remark}
                            size="small"
                            label={<FormattedMessage id="setting.blacklist.remark" />}
                        />
                    </Grid>
                    {expanded && (
                        <>
                            ...
                            <Grid item xs={12} sm={6}>
                                <CustomDateTimeRangePicker
                                    ...
                                    label={intl.formatMessage({ id: 'setting.blacklist.created-date' })}
                                />
                            </Grid>
                        </>
                    )}
                    <Grid item xs={12} sm={3} sx={{ display: 'flex', justifyContent: 'flex-end', marginLeft: 'auto' }}>
                        <AddButton
                            onClick={() => handleAddModal()}
                            tooltipTitle={intl.formatMessage({ id: 'setting.blacklist.add-blacklist' })}
                        />
                    ...
                    <Grid item sm={6} textAlign="left" sx={{ alignSelf: 'center' }}>
                        <Typography>
                            {formikInitialValues['id'] === 0
                                ? intl.formatMessage({ id: 'setting.blacklist.add-blacklist' })
                                : intl.formatMessage({ id: 'setting.blacklist.edit-blacklist' })}
                        </Typography>
                    </Grid>
                        ...
                        <Formik
                            ...
                            onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
                                try {
                                    await axiosServices
                                        .post(
                                            `${envRef?.API_URL}/admin/blacklist/edit`,
                                            {
                                                ip: values.ip,
                        ...
                        >
                            {({
                                ...
                                <form noValidate onSubmit={handleSubmit}>
                                        ...
                                        <Grid item xs={12} sm={6}>
                                            ...
                                            >
                                                <InputLabel htmlFor="outlined-adornment-real-name-add">
                                                    {<FormattedMessage id="setting.blacklist.ip" />}
                                                </InputLabel>
                                                <OutlinedInput
                                    ...
                                    <FormattedMessage id="setting.blacklist.ip-info1" />
                                    <br />
                                    <FormattedMessage id="setting.blacklist.ip-info2" />
                                    <br />
                                    <FormattedMessage id="setting.blacklist.ip-info3" />
                                    <br />
                                    &nbsp;
                                    <Grid container alignItems="center" spacing={2}>
                                        <Grid item xs={12} sm={6}>
                                        ...
                                        <Grid item xs={12}>
                                            ...
                                            >
                                                <InputLabel htmlFor="outlined-adornment-remark-add">
                                                    {intl.formatMessage({ id: 'setting.blacklist.remark' })}
                                                </InputLabel>
                                                <OutlinedInput
                                                    multiline
            ...
            <GeneralDialog
                confirmFunction={() => handleDelete(selectedToDelete!)}
